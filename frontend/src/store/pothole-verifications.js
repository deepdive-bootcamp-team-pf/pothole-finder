...
export const { getAllPotholeVerifications, setNewPotholeVerification } =
  slice.actions;
...
export const fetchPotholeVerificationsByPotholeId =
  () => async (dispatch, getState) => {
    const { data } = await httpConfig("/apis/pothole-verification");
    dispatch(getAllPotholeVerificationsByPotholeId(data));
    const potholeIds = _.uniq(
      _.map(getState().potholes, "potholeVerificationPotholeId")
    );
