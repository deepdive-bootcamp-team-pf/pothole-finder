...
export function GetMarker(props) {
  ...
  const [lng, setLng] = useState(-106.60473);
  ...
  const toPotholeSubmission = () => {
    navigate("pothole-submission-page", {
      state: { lat: location.lat, lng: location.lng },
    });
  };
...
export default function MapFunction(props) {
  ...
  const potholes = useSelector((state) =>
    state.potholes ? state.potholes : []
  );
  const dispatch = useDispatch();
  const effects = () => {
