...
export function PotholeSubmissionForm(props) {
  ...
  const validator = Yup.object().shape({
    potholeDescription: Yup.string().max(
      512,
      "Pothole description must be less than 512 characters."
    ),
    ...
    photoName: Yup.string()
      .optional()
      .max(32, "Your photo name is too long. It can only be 32 characters."),
  });
  ...
  function submitPotholePhoto(values, { resetForm, setStatus }) {
    ...
    httpConfig.post(`/apis/pothole/`, pothole).then((reply) => {
    ...
    function uploadImage(image, potholeId) {
      httpConfig.post(`/apis/image-upload/`, image).then((reply) => {
        let { message, type } = reply;
    ...
    <>
      <Form onSubmit={handleSubmit} className="p-3 form-text">
        ...
        <div className="mx-auto d-flex justify-content-center">
          {selectedImage !== null ? (
            <img className="restrain-image" src={selectedImage} alt="pothole" />
          ) : (
            ""
          )}
        </div>
        ...
        <Form.Group className="mb-3" controlId="photoName">
          ...
          {errors.photoName && touched.photoName && (
            <>
              <div className="alert alert-danger">{errors.photoName}</div>
            </>
          )}
        </Form.Group>
        <br />
    ...
    <Form.Group className="mb-3" {...getRootProps()}>
      <InputGroup size="lg" className="">
        <div className="mt-3 form-control-div mx-auto d-flex bg-light justify-content-center align-items-center border rounded w-50 py-3 px-3">
          ...
          />
          {isDragActive ? (
            <span className="align-items-center text-secondary">
              Drop image here
            </span>
          ) : (
            <span className="align-items-center text-secondary opacity-25">
              Drag and drop image, <br /> or click to choose from device.
            </span>
          )}
        </div>
      </InputGroup>
