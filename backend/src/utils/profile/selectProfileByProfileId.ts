...
export async function selectProfileByProfileId(profileId: string): Promise<Profile|null> {
    ...
    const mysqlQuery = 'SELECT BIN_TO_UUID(profileId) AS profileId, profileEmail, profileFirstName, profileLastName, profileUsername FROM profile WHERE profileId = UUID_TO_BIN(:profileId)'
