...
export async function activationController(request: Request, response: Response): Promise<Response> {
    try {
        ...
        const activationFail = (): Response => response.json({
            status: 400,
            data: null,
            message: 'Account activation failed, this account may have already been activated.'
        })
        ...
        return (profile != null) ? await activationSuccess(profile) : activationFail()
    } catch (e: any) {
        return response.json({
            status: 500,
            data: null,
