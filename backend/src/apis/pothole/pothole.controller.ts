...
export async function getAllPotholesController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
...
export async function getPotholeByPotholeIdController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
...
export async function getPotholesByPotholeProfileIdController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
...
export async function postPotholeController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
    const { potholeSeverity, potholeDescription, potholeLat, potholeLng } =
      request.body;
...
export async function deletePotholeController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
    ...
    return targetedPothole !== null &&
      (targetedPothole.potholeProfileId as string) === profileIdFromSession
      ? deleteSucceeded(response, targetedPothole)
      : deleteFailed(response);
  } catch (error) {
    return response.json({
      photo: 500,
      data: null,
      message: "Error deleting pothole. Please try again.",
    });
  }
}
...
function deleteFailed(response: Response): Response {
  return response.json({
    status: 400,
    message:
      "Input incorrect or you do not have permission to delete this pothole.",
    data: null,
  });
}
