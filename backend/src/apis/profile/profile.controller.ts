...
export async function deleteProfileController(
  ...
  try {
    ...
    return loggedInProfile !== null && loggedInProfile.profileId === profileId
      ? deleteSucceeded(response, loggedInProfile)
...
function deleteFailed(response: Response): Response {
  return response.json({
    status: 400,
    message:
      "Input incorrect or you do not have permission to delete this profile.",
    data: null,
  });
}
...
export async function getAllProfileController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
...
export async function getProfileByProfileIdController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
...
export async function putProfileController(
  request: Request,
  response: Response
): Promise<Response> {
  try {
    ...
    const performUpdate = async (
      ...
      const previousProfile = await selectProfileByProfileId(
        profileIdFromSession
      );
      const newProfile: PartialProfile = { ...previousProfile, ...profile };
    ...
    return profileId === profileIdFromSession
      ...
      : updateFailed("Please login to update profile.");
  } catch (e) {
    return response.json({
      status: 500,
