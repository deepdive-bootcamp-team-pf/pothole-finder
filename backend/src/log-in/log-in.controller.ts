...
export async function logInController(
  ...
  try {
    ...
    const profile: Profile | null = await selectProfileByProfileUsername(
      profileUsername
    );

    return profile !== null &&
      (await validatePassword(profile.profileHash, profilePassword))
...
export async function getProfileByProfileUsername(
  request: Request,
  response: Response
): Promise<Response> {
  try {
